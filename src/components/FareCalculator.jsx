...
export default function FareCalculator() {
    ...
    const [cashlessFare, setCashlessFare] = useState(0.00);
    const [cashFare, setCashFare] = useState(0.00);
    ...
    const getCashFare = (selectedStartingStation, selectedArrivalStation) => {
        ...
        const startingStationData = fareTable.find(entry => entry.station === selectedStartingStation.label);

        console.log("Starting Station Data:", startingStationData);
        
        if (startingStationData && startingStationData.fares[selectedArrivalStation.label]) {
            ...
            setErrorMessage("Error - Starting or arrival station not found in fare table data");
            setCashFare(0.00);// Reset fare when station data is not found in fare table
            return 0.00;
        }
    };
    ...
    const getCashlessFare = (selectedStartingStation, selectedArrivalStation) => {
        ...
        if (startingStationData && startingStationData.fares[selectedArrivalStation.label]) {
            ...
            console.error("Starting or arrival station not found in fareTable");
            setErrorMessage("Error - Starting or arrival station not found in fare table data");
            setCashlessFare(0.00); // Reset fare when station data is not found in fare table
            return 0.00;
        }
    };
    ...
    useEffect(() => {
        if (startingStation && arrivalStation) {
          ...
          setCashFare(0.00);
          setCashlessFare(0.00);// Reset fare when stations are not selected
        }
    ...
    return (
        <div className="m-4">
            ...
            <div className="p-4">
                <p className="border border-light rounded p-2 text-center">Cash: {!errorMessage ? cashFare.toFixed(2) : <p className="text-danger">{errorMessage}</p>}</p>
                {/* <p>{errorMessage && <p className="text-danger">{errorMessage}</p>}</p> */}
                <p className="border border-light rounded p-2 text-center">Cashless: {cashlessFare.toFixed(2)} </p>
                <p className="border border-light rounded p-2 text-center">Concession: {(cashlessFare / 2).toFixed(2)} (dummy data due a lack of access to concession fare data)</p>
                <p>Disclaimer: The fare table in use only contains data for certain stations for demo purpose; error message will be shown if a station is not found in the fare table.</p>
            </div>
